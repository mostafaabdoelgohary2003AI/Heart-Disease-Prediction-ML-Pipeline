...
def load_real_trained_models():
    ...
    try:
        ...
        # Try to load your actual trained model (Random Forest - best performer)
        # But handle version incompatibility gracefully
        ...
        _ = model.predict_proba(scaler.transform(test_input))
...
if page == "🏠 Home":
    ...
    with col2:
        ...
        # Cross-tabulation of clusters vs actual labels - simplified approach
        try:
            cluster_disease = pd.crosstab(cluster_labels, y)
            
            # Simple bar chart showing cluster sizes
            cluster_counts = pd.Series(cluster_labels).value_counts().sort_index()
            fig_cluster_disease = px.bar(
            ...
            fig_cluster_disease = px.bar(
                ...
                y=[50, 50],
                title='Cluster Distribution (Demo)'
            )
